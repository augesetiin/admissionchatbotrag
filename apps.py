...
Answer:"""
    ...
    return RetrievalQA.from_chain_type(
        llm=_llm, chain_type="stuff", retriever=retriever,
...
You can find official documents on the [SIES Admissions Portal](https://siesascn.edu.in/admissions)."""
                    ...
                    try:
                        result = st.session_state.qa_chain.invoke({"query": prompt_clean})
                        raw_response = result["result"]
                        if not raw_response.strip() or len(raw_response.strip()) < 10 or any(phrase in raw_response.lower() for phrase in ["i don't have", "no information", "not in the context"]):
